...
func (c *Client) readPump() {
	...
	for {
		...
		hlog.Infof("Received WS message from client %d: %+v", c.id, inc)
		...
		if inc.Action == "status_update" {
			err = hub.msgUseCase.UpdateStatus(context.Background(), toID, c.id, inc.Status)
			if err != nil {
				hlog.Errorf("failed to update status: %v", err)
				continue
			}
		...
		_, err = hub.msgUseCase.SendMessage(context.Background(), c.id, toID, msgType, inc.Message)
		if err != nil {
			hlog.Errorf("failed to save message: %v", err)
			continue
		}
